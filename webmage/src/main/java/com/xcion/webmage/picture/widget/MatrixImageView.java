...
//            mScaleCenter.set(detector.getFocusX(), detector.getFocusY());
    ...
    private GestureDetector.OnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        public void onLongPress(MotionEvent e) {
            if (mLongClick != null) {
                mLongClick.onLongClick(MatrixImageView.this);
            }
        }
    ...
    public FromInfo getInfo() {
        RectF rect = new RectF();
        int[] p = new int[2];
        getLocation(this, p);
        rect.set(p[0] + mImgRect.left, p[1] + mImgRect.top, p[0] + mImgRect.right, p[1] + mImgRect.bottom);
        return new FromInfo(rect, mImgRect, mWidgetRect, mBaseRect, mScreenCenter, mScale, mDegrees, mScaleType);
    }

    public static FromInfo getImageViewInfo(ImageView imgView) {
        int[] p = new int[2];
        ...
        return new FromInfo(rect, imgRect, widgetRect, baseRect, screenCenter, 1, 0, imgView.getScaleType());
    }
     ...
     */
    public void animaFrom(FromInfo info) {
        if (isInit) {
            reset();

            FromInfo mine = getInfo();
    ...
    public void animaTo(FromInfo info, Runnable completeCallBack) {
        if (isInit) {
            mTranslate.stop();
